        ...
        orderItemDiv.innerHTML =
            `<img src="${book.bookImage}" alt="Product Image">
                <div class="order-product-info-description">
                    ...
                    <p>Giá: ${formatNumber(book.bookPrice)}</p>
                </div>`;

        orderProductList.appendChild(orderItemDiv);
    });

    document.getElementById('productPrice').innerText = `${formatNumber(order.orderTotal)} đ`;
    document.getElementById('totalPrice').innerText = `${formatNumber(order.orderTotal + 25000)} đ`;
}